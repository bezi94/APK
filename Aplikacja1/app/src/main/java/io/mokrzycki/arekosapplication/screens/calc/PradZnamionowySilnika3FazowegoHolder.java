...
class PradZnamionowySilnika3FazowegoHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    public PradZnamionowySilnika3FazowegoHolder(ViewGroup parent) {
    ...
    public void bind() {
        textView0.setText("Prąd Znamionowy Silnika 3 Fazowego");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button1: {
                ...
                float prad = (float) (moc_znamionowa / ((Math.sqrt(3) * napiecie3f) * sprawnosc * wspolczynnik));
                float prad_pop = FloatUtils.fixLength(prad, 2);

                if (prad_pop < 13) {
                    textView5.setText(String.format("%sA. Przewod 1,5mm i zab. 13A.", prad_pop));
                } else if (prad_pop < 16) {
                    textView5.setText(String.format("%sA. Przewod 2,5mm i zab. 16A.", prad_pop));
                } else if (prad_pop < 25) {
                    textView5.setText(String.format("%sA. Przewod 4mm i zab. 25A.", prad_pop));
                } else if (prad_pop < 32) {
                    textView5.setText(String.format("%sA. Przewod 6mm i zab. 32A.", prad_pop));
                } else if (prad_pop < 40) {
                    textView5.setText(String.format("%sA. Przewod 10mm i zab 40A.", prad_pop));
                } else if (prad_pop < 50) {
                    textView5.setText(String.format("%sA. Przewod 16mm i zab. 50A.", prad_pop));
                } else if (prad_pop < 63) {
                    textView5.setText(String.format("%sA. Przewod 25mm i zab. 63A.", prad_pop));
                } else if (prad_pop < 80) {
                    textView5.setText(String.format("%sA. Przewod 35mm i zab. 80A.", prad_pop));
                } else {
                    textView5.setText(String.format("Wynik: %sA. Mam wiedze tylko do 80 A.", prad_pop));
                }
            }
            break;
        }
